...
1. **Qualifier-round heavy favorites on the match line** (revised by the Phase 0 extended backtest — see §8b). The initial two-month window suggested a tour-wide favorite-longshot bias (90¢+ favorites winning ~4 points above implied); the full 13-month history refutes the tour-wide version (taker EV negative at every price floor) but confirms a concentrated pocket: **favorites ≥85¢ in qualifying matches returned +4.1¢/contract net of fees (95.8% hit, n=238, CI [+1.4, +6.3]), ~20 trades/month**, ATP stronger than WTA. The inefficiency tracks attention, not contract structure.
...
## 8b. Phase 0 addendum (Jul 19): the extended backtest reverses the FLB headline

Extending the match-line calibration through Kalshi's `/historical/` tier (13 months, Jun 2025 – Jul 2026, n=6,814 pre-match observations at T−2h) **kills the tour-wide favorite trade** that §6D flagged:

- Overall calibration is good: every price bucket within ±2¢ of implied. The May–July "+4 points on 90¢+ favorites" was a window artifact; on the full sample 0.90–0.93 shows −0.2¢, 0.93–0.96 +1.9¢, 0.96+ +0.9¢ — all inside CIs.
- Realized taker EV of "buy the favorite at ask ≥ floor" is **negative at every floor** (−0.5¢ to −1.6¢/contract, n up to 1,174). 2025 was significantly negative (−3.7¢); 2026 is ≈ 0 — the market has tightened as volume grew.
- **One pocket survives: qualifier heavy favorites.** In qualifying-round matches (~21% of listings, 2026 only — Kalshi didn't quote quals pre-match in 2025): floor 0.85 → 238 trades, 95.8% hit, **+4.1¢/contract net [+1.4, +6.3]**; floor 0.90 → 130 trades, 98.5% hit, +3.6¢ [+1.1, +5.3]. ATP carries it (60/60 at 0.90+, +5.0¢); WTA is positive but not significant. ~20 trades/month, asks clustered 92–96¢.

Interpretation: the *match-line* inefficiency lives exactly where the props inefficiency lives — in the markets nobody watches. Attention, not contract structure, is the gradient. Caveats before trading it: 7 months of data, no 2025 holdout, multiple-comparisons discount (this is one cell of ~10 examined), and thin qualifier books mean displayed-ask fills are optimistic. Verdict: worth a narrow paper pod with sequential monitoring; not yet worth capital.

## 8c. Attention-thesis test on Challenger/ITF (Jul 20) — **REFUTED**

§8b proposed a mechanism: mispricing tracks *attention*, and qualifying
rounds are the least-watched markets. That mechanism makes a sharp
out-of-sample prediction — Challenger and ITF markets (structurally
lower-attention than tour main draws) should show the same or a
stronger effect. Tested on a seeded 9,000-event sample (3,253 usable
pre-match observations at T−2h):

| cohort | n | hit | EV/contract, net | 95% CI |
|---|---|---|---|---|
| **tour quals** (P-015 basis) | 238 | .958 | **+4.06¢** | [+1.44, +6.34] |
| tour main draw | 936 | .901 | −2.06¢ | [−4.04, −0.14] |
| challenger, all | 575 | .910 | −1.44¢ | [−3.75, +0.73] |
| **challenger quals** | 282 | .911 | **−1.98¢** | [−5.24, +1.15] |
| challenger main | 293 | .908 | −0.91¢ | [−4.19, +2.20] |
| ITF (main draw) | 312 | .910 | −2.33¢ | [−5.48, +0.59] |
| pooled: all quals | 520 | .933 | +0.78¢ | [−1.44, +2.80] |

**The prediction fails.** Lower-attention tours are *negative*, not
positive. Most damaging: **challenger qualifiers — the nearest possible
out-of-sample cohort, same round type, adjacent tour — return −2.0¢.**
If "qualifying rounds are underpriced" were the true effect, this is
precisely where it should have replicated, and it did not.

Two readings, and the honest answer is we cannot yet distinguish them:

1. **The tour-qual result is noise.** It survives Bonferroni over ~14
   examined slices (z=3.01, p=0.0013 vs threshold 0.0036) and is stable
   across a temporal split (first half +3.1¢, second half +5.0¢), so it
   is not a trivially fragile artifact. But a genuine effect with no
   working mechanism, that fails to replicate in the most similar
   cohort, is exactly what a surviving false positive looks like.
2. **The effect is real but specific to tour-level quals** — and
   *attention is the wrong explanation for it.* The tour-qual and
   challenger-qual cohorts differ significantly from each other
   (Δ=+6.0¢, z=2.85), so this is not one effect diluted by pooling;
   something separates them. Plausible candidates we have NOT tested:
   tour quals draw sharper flow than challengers (better-known players
   entering qualifying), or the surviving cohort is dominated by a
   handful of high-volume events.

**Consequences, applied:**

- **Do not build P-015b.** Pooling lower tours drops the edge to
  +0.78¢ with a CI spanning zero — expansion would dilute a possibly-real
  effect with demonstrably negative cohorts, while *appearing* to solve
  the sample-size problem. The volume gain (34/mo → 161/mo) is real but
  buys statistical power over a null.
- **Downgrade the P-015 prior.** The pod stays in paper under the
  §P015_DECISION_RULE thresholds unchanged (changing them now would be
  the exact rationalization the rule forbids), but the expectation
  should be KILL at the 120-trade checkpoint, not promotion.
- **The mechanism question is now the research question.** If tour quals
  are genuinely different, the next test is *why* — not more tours.
