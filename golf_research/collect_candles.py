...
def throttle():
    with rate_lock:
        now = time.time()
        wait = last[0] + 0.15 - now   # ~6.6 req/s
...
order = ["KXGOLFTOURN","KXPGAH2H","KXPGA3BALL","KXPGATOP10","KXPGATOP5","KXPGATOP20","KXPGAMAKECUT","KXPGATOP40","KXPGAR1LEAD","KXPGAR2LEAD","KXPGAR3LEAD"]
done = set()
if os.path.exists('candles.jsonl'):
    for line in open('candles.jsonl'):
        try: done.add(json.loads(line)['ticker'])
        except: pass
f = open('candles.jsonl','a')
count = [len(done)]
def work(args):
    s, m = args
    t = m['ticker']
    try:
        ...
        rec = {'ticker': t, 'series': s, 'event': m.get('event_ticker'), 'result': m.get('result'),
               'open_time': m['open_time'], 'close_time': m['close_time'], 'candles': c.get('candlesticks', [])}
    except Exception as e:
    ...
    with lock:
        ...
        count[0] += 1
        if count[0] % 500 == 0:
            f.flush(); print(count[0], s, flush=True)

jobs = [(s, m) for s in order for m in d.get(s, []) if m['ticker'] not in done]
