...
THE SAMPLE THIS POD WILL BE VALIDATED ON IS BIASED. READ THIS FIRST.
────────────────────────────────────────────────────────────────────
...
**The rate cannot be raised.**  128 cumulative 429s, 48 in the last 24 h,
with the daemon self-throttling to 0.6-1.2 req/s.  The limiter is the
exchange, not the config, so this is not a bug to fix before the backtest
— it is a permanent property of the sample.

Any report produced from this module's replay must state that bias **in
its own words, in the result section, not in a footnote.**  It is written
here, in the pod's own file, on the night the code landed and before any
number exists — so that when it becomes inconvenient it is already on the
record.  See ``research/REPORT_P018_Cherrypick_2026-07-29.md`` and
``research/REPORT_Data_Readiness_2026-07-27.md``.

STATUS: INERT.  No engine, no runner, no systemd unit.  Not in
``src/pods/``, carries no ``@register_pod``, ``enabled: false`` in
``config_multi_pod.yaml`` and absent from ``pods.active`` — four
independent reasons nothing here can trade.  It ships only after the
backtest kill-gate (spec §8 gate #1) shows the surprise gate actually
separates fade from expected/normal fills, and only after a
pre-registered decision rule exists.
"""
