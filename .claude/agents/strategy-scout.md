...
event could change the conclusion. Use `defer` only for a named external
`blocking_fact` that will arise without research effort. Missing collection,
data engineering, modelling, or implementation is `needs_work`, with a bounded
`next_action` and no calendar recheck. Do not submit either artifact or modify
the running system.
